...
class Mongo {
    ...
    async saveApplication(name, path, version){
        try {
            // const application = await this.getApplication({ name: name })
            // if (application && application[0]) {
                // return await this.updateMonitor(monitors[0], data)
            // }
            console.log("Before save to DB - in saveApplication")
            const doc = await this._applicationStorage.create({"name":name, "path":path, "version":version});
