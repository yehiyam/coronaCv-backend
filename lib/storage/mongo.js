...
class Mongo {
    ...
    async getMonitorImage({ monitorId }) {
        try {
            ...
            if (doc && doc[0]) {
                ...
                if (file) {
                    return { file, imageId: image.imageId, monitorId, timestamp: image.timestamp }
                }
                else return { imageId: image.imageId, monitorId, timestamp: image.timestamp }
            }
            return null;
        } catch (error) {
            console.error('saveMonitorImage error: ' + error)
    ...
    async updateMonitor(doc, { segments, imageId, ...data }) {
        ...
        doc.imageId = imageId
        doc.monitorImage = data.monitorImage;
        doc.monitorId = data.monitorId;
        doc.patientImage = data.patientImage;
        doc.patientId = data.patientId;
        doc.roomImage = data.roomImage;
