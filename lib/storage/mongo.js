...
class Mongo {
  ...
  async getMonitorImageForOcr({ monitorId, imageId }) {
    try {
      ...
      if (doc && doc[0]) {
        ...
        if (file) {
          return {
            file, imageId: image.imageId, monitorId, timestamp: image.timestamp, ocrResults,
          };
        }
        return {
          imageId: image.imageId, monitorId, timestamp: image.timestamp, ocrResults,
        };
      }
      return null;
    } catch (error) {
  ...
  static async updateMonitor(doc, { segments, imageId, ...data }) {
    if (segments) {
