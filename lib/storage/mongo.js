...
class Mongo {
    ...
    _createSchemas() {
        ...
        const monitorOcrFromDeviceSchema = new mongoose.Schema({
            monitorId: { type: String, index: true },
            imageId: { type: Number, index: true },
            timestamp: { type: Date, index: true },
            segments: { type: [{}] }
        ...
        this._monitorOcrFromDevice = mongoose.model('monitorOcrFromDevice', monitorOcrFromDeviceSchema);
    ...
    async saveMonitorOcrFromDevice(data) {
        try {
            const doc = await this._monitorOcrFromDevice.create(data);
            await doc.save();
            return 'saved';
        } catch (error) {
    ...
    async getMonitorOcrFromDevice({ monitorId, imageId }) {
        try {
            let doc;
            if (imageId != null) {
                doc = await this._monitorOcrFromDevice.find({
                    monitorId, imageId
