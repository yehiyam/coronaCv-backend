...
const monitorImages = () => {
  ...
  router.post('/', asyncHandler(async (req, res, next) => {
    const { body } = req;
    const imageId = parseInt(req.headers['x-image-id'] || '0,', 10);
  ...
  router.get('/:monitorId', async (req, res, next) => {
    ...
    cache.lastRequestedImageForMonitor[monitorId] = Date.now();

    try {
      const ret = await monitorImageGetLatest({
        monitorId, imageId, showOcr, withOcr: false,
      });
      if (!ret || !ret.file) {
        res.status(404);
        res.end();
