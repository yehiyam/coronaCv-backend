...
const { monitorList, monitorGet, monitorPost, monitorDelete, monitorGetImages } = require('../services/monitor');
...
const monitorSetup = () => {
    ...
    router.get('/images/:monitorId', async (req, res, next) => {
        try {
            ...
            const ret = await monitorGetImages({ monitorId });
            if (!ret) {
                res.status(404)
                res.end();
            } else {
                if (html){
                    ...
                    ret.forEach(l=>{
                        lines.push('<tr>')
                        lines.push(`<td><img src="/monitor_image/${monitorId}?imageId=${l.imageId}" title=${l.imageId} alt=${l.imageId}></td>`)
                        lines.push('</tr>')
                    })
                    lines.push('</table>')
                    lines.push('</body>')
