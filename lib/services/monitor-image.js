...
const monitorImageSaveOcr = async (data) => {
  ...
  await db.saveMonitorImage({
    imageId, monitorId, timestamp, ocrResultsFromDevice: segments,
  });
  return true;
};
const monitorImagePost = async (data) => {
  ...
  if (!monitor.monitorId) {
    await db.saveMonitor({ monitorId, timestamp });
  }
  ...
  try {
    if (alignedFile && Buffer.isBuffer(alignedFile)) {
      ...
      ocred = await cv.ocr({
        segments, image: encodedFile, imageId, monitorId,
      });
    } else {
  ...
  // save to database
  await db.saveMonitorImage({
    imageId, monitorId, timestamp, file: alignedFile, ocrResults: ocred,
  });
  const ret = {
    monitorId,
...
const monitorImageGetLatest = async ({
  monitorId, imageId, showOcr, withOcr = true,
}) => {
  ...
  if (showOcr) {
    ...
    if (imageFromDb.file && Buffer.isBuffer(imageFromDb.file)) {
      ...
      const ocred = await cv.showOcr({ segments, image: encodedFile });
